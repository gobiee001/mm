...
::         adb is needed both for the gadget port forward and for the
::         environment's own crash recovery (it relaunches the app).
where adb >nul 2>&1
if errorlevel 1 (
...
:: Step 2: Forward the Frida gadget port. Skipped in --mock mode, which needs no
::         device at all.
...
:: Step 3: Locate Python. Training\ sits at the same depth under MM_GYm\ as
::         python_gym_Wrapper\, so the same probe order as run_gym_demo.bat works.
...
:: Step 4: Run the trainer as a module from MM_GYm\, so that both Training and
::         python_gym_Wrapper resolve as top-level packages.
pushd "%~dp0.."
...
if not "!EXITCODE!"=="0" (
    ...
    echo       run_training.bat --mock --total-timesteps 8192
)
...
echo.
echo To stop a long run cleanly, create a STOP file in the run folder:
echo   type nul ^> MM_GYm\models\run_^<timestamp^>\STOP
echo.

:: Step 5: TensorBoard reminder. Probe only - never install anything here.
where tensorboard >nul 2>&1
if errorlevel 1 (
    echo [i] tensorboard is not installed, so this run wrote progress.csv only.
    echo     Install it from your internal artifact registry to get the dashboard;
    echo     no code change is needed afterwards.
) else (
