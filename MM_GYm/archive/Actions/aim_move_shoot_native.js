{
    ...
    // bypassHold=true skips the hold latch entirely - used by the continuous
    // self-test sweep, where every tick is a deliberate, smooth change, not a
    // conflicting new command that needs debouncing.
    function updateActions(mx, my, ax, ay, shoot, normalized, bypassHold) {
        ...
        if (bypassHold || !currentActions.has_action || !changed || holdExpired) {
    ...
    if (moduleBase !== null) {
        ...
        // ---- Self test: step through the full [-1, 1] range, one axis at a
        // time (the other held at 0 so per-axis magnitude response is
        // observable without diagonal movement confounding it). Each value is
        // held for MOVE_HOLD_MS before advancing - watch it settle, don't just
        // flicker past it.
        const SWEEP_STEPS = [-1.0, -0.75, -0.5, -0.25, 0.0, 0.25, 0.5, 0.75, 1.0];

        if (TEST_MODE === "CYCLE") {
            let sweepPhase = 0;   // 0 = sweeping X (Y held at 0), 1 = sweeping Y (X held at 0)
            let stepIdx = 0;
            setInterval(function () {
                if (TEST_MODE !== "CYCLE") return;
                const v = SWEEP_STEPS[stepIdx];
                const x = sweepPhase === 0 ? v : 0.0;
                const y = sweepPhase === 1 ? v : 0.0;
                updateActions(x, y, x, y, 0, false);

                stepIdx++;
                if (stepIdx >= SWEEP_STEPS.length) {
                    stepIdx = 0;
                    sweepPhase = (sweepPhase + 1) % 2;
                }
            }, MOVE_HOLD_MS);
        }
    }
